...
namespace EasySave_Interface.Model
{
    public class Backup
    {
        ...
        private void CompareandWrite(string sourcePath, string comPath, int i, string typeBackup, bool final)
        {
            ...
            {
                if (this.viewModel.settings.extensions.Contains(file.Extension))
                {
                    ...
                    string tempPath = Path.Combine(dirdest + viewModel.settings.compressType);
                    //Play, Pause, Stop
                    ...
                    else 
                    {
                        ...
                        using (FileStream zipToOpen = new FileStream(tempPath , FileMode.OpenOrCreate))
                        {
                            using (ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update))
                            {
                                ZipArchiveEntry readmeEntry = archive.CreateEntryFromFile(file.FullName, file.Name);
                            }
                        }
                    }
                    stopWatch.Stop();
